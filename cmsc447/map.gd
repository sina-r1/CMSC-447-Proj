...
func _ready():
	...
	var scale_factor = resolution.x / map_image.get_size().x
	map_sprite.position = resolution / 2
	map_sprite.scale = Vector2(scale_factor, scale_factor)
	
	background_color.size = resolution
